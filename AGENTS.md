...
This repository is currently documentation-first. Active assets live in `documents/` for architecture and implementation guidance, and `prompts/` for staged notebook-generation prompts. There is no `src/` or `tests/` tree yet, but contributors should follow the planned layout in `documents/IMPLEMENTATION_PLAN.md`: `src/` for pipeline stages, `tests/` for Pytest coverage, `examples/` for sample inputs, and `runs/` for generated artifacts and logs.
...
There is no build system yet. Use these commands while working on the current repo:
